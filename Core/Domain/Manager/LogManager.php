<?php
	...
	class LogManager extends DomainManager
	{
		...
		public function GetByID($id) {
			...
			$sqlObject = "SELECT log_id, status, object_table, object_id, user_id, type, timestamp FROM log WHERE status = 100 AND log_id = :id";
		...
		public function GetByObjectAndType(Domain $object, $type) {
			...
			$sqlObject = "SELECT log_id, status, object_table, object_id, user_id, type, timestamp FROM log WHERE status = 100 AND object_table = :table AND object_id = :object AND type = :type ORDER BY timestamp DESC";
			$stmObject = $db->Prepare($sqlObject);
